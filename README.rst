...
Unlike computation graph based engines such as Dask or PySpark, Pyflow is not meant to be a parallel data processor, or to change the way computation resources are used. Instead, it is meant to be a light weight tool for code organization in the form of DAG and for graph visualization. It's computation graph logic relies on pure Python -- as doubly linked lists -- and not on any other bulky third party packages such as NetworkX.
